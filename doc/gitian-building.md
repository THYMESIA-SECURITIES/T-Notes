...
*Setup instructions for a gitian build of T-Notes using a VM or physical system.*

Gitian is the deterministic build process that is used to build the T_Notes
...
to the T_Notes GitHub release page.
...
- [Building T-Notes](#building-t_notes-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/t_notes-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/t_notes-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/t_notes-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/t_notes-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/t_notes-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/t_notes-win-${VERSION%\.*}-build.assert
```
...
You can also mail the files to Fuzzbawls (fuzzbawls@t_notes.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/t_notes-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/t_notes-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/t_notes-osx-*-build.assert
```
