...
- [Development guidelines](#development-guidelines)
    - [General T-Notes](#general-t_notes-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
T-Notes uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play TNCs (tTNC)" on the test network, if you
...
T-Notes is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/THYMESIA-SECURITIES/T-Notes/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_t_notes
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_t_notes --log_level=test_suite
$ valgrind -v --leak-check=full src/t_notesd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_t_notes.coverage/index.html`.
```
...
- BitcoinMiner : Generates TNCs (if wallet is enabled).
...
However, in open source software such as T-Notes, where everyone uses
...
pay attention to for reviewers of T-Notes code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: T-Notes uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against T-Notes so that they can be integrated
...
file descriptors it will cause problems with T_Notes's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:THYMESIA-SECURITIES/T-Notes.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `t_notes-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `t_notes-cli` and the GUI debug console use this table to determine how to
