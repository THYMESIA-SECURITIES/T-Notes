UNIX BUILD NOTES
====================
Some notes on how to build T-Notes in Unix.

Note
---------------------
Always use absolute paths to configure and compile T-Notes and the dependencies,
...
This will build t_notes-qt as well, if the dependencies are met.
...
memory available when compiling T-Notes. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~t_notes/+archive/t_notes).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:t_notes/t_notes
    sudo apt-get update
...
To build T-Notes without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build t_notes-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a t_notes-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip t_notesd" to strip the debug
...
Security
--------
To help make your T-Notes installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, T-Notes should be built with a non-executable stack
...
When the intention is to run only a P2P node without a wallet, T-Notes may be compiled in
