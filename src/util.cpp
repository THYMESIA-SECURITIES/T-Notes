...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\T-Notes
// Windows >= Vista: C:\Users\Username\AppData\Roaming\T-Notes
// Mac: ~/Library/Application Support/T-Notes
// Unix: ~/.t-notes
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "T-Notes";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\T-NotesParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\T-NotesParams
    // Mac: ~/Library/Application Support/T-NotesParams
    // Unix: ~/.t-notes-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "T-NotesParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "T-NotesParams";
#else
    // Unix
    return pathRet / ".t-notes-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/t_notes/sapling-spend.params";
        sapling_output = "/usr/share/t_notes/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/t_notes/sapling-spend.params";
            sapling_output = "/usr/local/share/t_notes/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
void ArgsManager::ReadConfigFile()
{
    ...
    if (!streamConfig.good()) {
        // Create empty t-notes.conf if it does not exist
...

    {
        ...
        for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
            // Don't overwrite existing settings so command line settings override t-notes.conf
