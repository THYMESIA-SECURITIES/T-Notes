...
CTncStake* CTncStake::NewTncStake(const CTxIn& txin)
{
    ...
    return new CTncStake(txPrev->vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CTncStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CTncStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CTncStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CTncStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CTncStake::GetUniqueness() const
{
    //The unique identifier for a TNC stake is the outpoint
...
const CBlockIndex* CTncStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CTncStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CTncStake::ContextCheck(int nHeight, uint32_t nTime)
{
