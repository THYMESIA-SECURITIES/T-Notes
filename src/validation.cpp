...
#if defined(NDEBUG)
#error "T_Notes cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in utnc) are considered zero fee (for relaying, mining and transaction creation)
                              ...
                              std::vector<COutPoint>& coins_to_uncache)
{
    ...
    if (!CheckTransaction(tx, state, false, fColdStakingActive, fSaplingActive))
...
{
    util::ThreadRename("t_notes-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> tncInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                tncInputs.push_back(stakeIn);
        }
        const bool hasTNCInputs = !tncInputs.empty();
        ...
        for (const auto& txIn : block.vtx) {
            ...
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasTNCInputs) {
                    ...
                    for (const CTxIn& tncIn : tncInputs)
                        if(tncIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const auto& txIn : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasTNCInputs) {
                            for (const CTxIn& stakeIn : tncInputs)
