...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select TNC Outputs to Spend" :
            "Select Shielded TNC to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::TNC;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show T_Notes address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
