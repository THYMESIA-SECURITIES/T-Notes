...
#include "qt/t_notes/settings/settingsinformationwidget.h"
#include "qt/t_notes/settings/forms/ui_settingsinformationwidget.h"
...
SettingsInformationWidget::SettingsInformationWidget(T_NotesGUI* _window,QWidget *parent) :
    ...
    ui(new Ui::SettingsInformationWidget)
{
    ...
    connect(ui->pushButtonFile, &QPushButton::clicked, [this](){
        if (!GUIUtil::openConfigfile())
            inform(tr("Unable to open t-notes.conf with default application"));
    });
