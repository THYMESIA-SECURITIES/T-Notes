...
#include "qt/t_notes/forms/ui_masternodeswidget.h"

#include "qt/t_notes/masternodewizarddialog.h"
#include "qt/t_notes/mninfodialog.h"
#include "qt/t_notes/mnrow.h"
...
MasterNodesWidget::MasterNodesWidget(T_NotesGUI* parent) : PWidget(parent),
        ...
        if (index.sibling(index.row(), MNModel::WAS_COLLATERAL_ACCEPTED).data(Qt::DisplayRole).toBool()) {
            ...
            inform(tr("Cannot start masternode, the collateral transaction has not been confirmed by the network yet.\n"
                      "Please wait few more minutes (masternode collaterals require %1 confirmations).")
                       .arg(MasternodeCollateralMinConf()));
        }
    }
}
    ...
    if (fs::exists(pathBootstrap)) {
        ...
        if (lineCopy.size() == 0) {
            lineCopy = "# Masternode config file\n"
                       ...
                       "# Example: mn1 127.0.0.2:14530 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0\n";
        }
