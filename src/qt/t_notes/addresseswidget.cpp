...
#include "qt/t_notes/addresseswidget.h"
#include "qt/t_notes/forms/ui_addresseswidget.h"
#include "qt/t_notes/addresslabelrow.h"
#include "qt/t_notes/addnewaddressdialog.h"
#include "qt/t_notes/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(T_NotesGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto tncAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(tncAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(tncAdd);
        if (walletModel->updateAddressBookLabels(tncAdd, label.toUtf8().constData(),
