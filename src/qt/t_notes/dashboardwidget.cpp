...
#include "qt/t_notes/dashboardwidget.h"
#include "clientmodel.h"
#include "guiutil.h"
#include "optionsmodel.h"
#include "qt/t_notes/forms/ui_dashboardwidget.h"
#include "qt/t_notes/qtutils.h"
#include "qt/t_notes/sendconfirmdialog.h"
#include "qt/t_notes/txrow.h"
#include "utiltime.h"
...
DashboardWidget::DashboardWidget(T_NotesGUI* parent) : PWidget(parent),
                                                                       ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareTnc, "square-chart-tnc");
    setCssProperty(ui->labelTnc, "text-chart-tnc");
    ...
    setCssProperty(ui->labelAmountTnc, "text-stake-tnc-disable");
    ...
    if (window)
        connect(window, &T_NotesGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::changeChartColors()
{
    ...
    if (isLightTheme()) {
        ...
        gridLineColorX = QColor(23, 16, 11);
        linePenColorY = gridLineColorX;
        backgroundColor = linePenColorY;
    }
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair TNC
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            tnc = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalTnc += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesTnc.append(tnc);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(33, 93, 148));
    ...
    set0->append(chartData->valuesTnc);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalTnc > 0) {
        setCssProperty(ui->labelAmountTnc, "text-stake-tnc");
    } else {
        setCssProperty(ui->labelAmountTnc, "text-stake-tnc-disable");
    }
    forceUpdateStyle({ui->labelAmountTnc});
    ui->labelAmountTnc->setText(GUIUtil::formatBalance(chartData->totalTnc, nDisplayUnit));
