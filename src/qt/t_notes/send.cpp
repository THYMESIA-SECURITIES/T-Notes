...
#include "qt/t_notes/send.h"
#include "qt/t_notes/forms/ui_send.h"
#include "qt/t_notes/addnewcontactdialog.h"
#include "qt/t_notes/qtutils.h"
#include "qt/t_notes/sendchangeaddressdialog.h"
#include "qt/t_notes/optionbutton.h"
#include "qt/t_notes/sendconfirmdialog.h"
#include "qt/t_notes/guitransactionsutils.h"
#include "qt/t_notes/loadingdialog.h"
...
SendWidget::SendWidget(T_NotesGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-tnc");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onTNCSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onTNCSelected(false);});
...
void SendWidget::refreshAmounts()
{
    ...
    QString labelAmountRemaining = GUIUtil::formatBalance( totalAmount, nDisplayUnit) + " " + type;
    QMetaObject::invokeMethod(this, "updateAmounts", Qt::QueuedConnection,
                              Q_ARG(QString, titleTotalRemaining),
                              Q_ARG(QString, GUIUtil::formatBalance(total, nDisplayUnit)),
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent TNCs.
    ...
    if (showCheckBox)
        ui->checkBoxDelegations->setToolTip(
                tr("Possibly spend coins delegated for cold-staking (currently available: %1").arg(
                        GUIUtil::formatBalance(delegationBalance, nDisplayUnit))
        );
}
    ...
    if (availableBalance > 0) {
        ...
        if (!ask(tr("Shield Coins"),
                 tr("You are just about to anonymize all of your balance!\nAvailable %1\nWith fee %2\n\n"
                    ...
                    "\n\nDo you want to continue?\n").arg(GUIUtil::formatBalanceWithoutHtml(recipient.amount, nDisplayUnit))
                                                     .arg(GUIUtil::formatBalanceWithoutHtml(nPayFee, nDisplayUnit))
                    )) {
            return;
        }
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield TNCs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent TNCs to shield."));
    }
}
...
void SendWidget::onTNCSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto tncAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(tncAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(tncAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(tncAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
