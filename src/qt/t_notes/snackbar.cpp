...
#include "qt/t_notes/forms/ui_snackbar.h"
#include "qt/t_notes/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(T_NotesGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &T_NotesGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
