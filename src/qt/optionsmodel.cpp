...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::TNC);
    ...
    if (settings.contains("nAnonymizet_notesAmount") || reset)
        gArgs.SoftSetArg("-anonymizet_notesamount", settings.value("nAnonymizet_notesAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in t_notes.cpp during shotdown to also remove the window positions
