...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter T-Notes address (e.g. %1)").arg("TaFpABdcahyP2RNVzoZzyGHbGjcgbVC4RV"));
...
{
    QRegularExpression rx("^(\\d{0,8})((\\.|,)\\d{1,6})?$");
...
{
    // return if URI is not valid or is no T_Notes: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::TNC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert t_notes:// to t_notes:
    //
    //    Cannot handle this later, because t_notes:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::TNC, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
fs::path static StartupShortcutPath()
{
    if (gArgs.GetBoolArg("-testnet", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "T_Notes (testnet).lnk";
    else if (gArgs.GetBoolArg("-regtest", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "T_Notes (regtest).lnk";

    return GetSpecialFolderPath(CSIDL_STARTUP) / "T_Notes.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for T_Notes*.lnk
...
{
    return GetAutostartDir() / "t_notes.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (!optionFile.good())
            return false;
        // Write a t_notes.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=T_Notes (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=T_Notes (regtest)\n";
        else
            optionFile << "Name=T_Notes\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the t_notes app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add t_notes app to startup item list
