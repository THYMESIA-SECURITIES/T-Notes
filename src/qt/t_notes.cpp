...
#include "qt/t_notes/splash.h"
#include "qt/t_notes/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("t-notes", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in t_notes.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in t_notes.qrc)
...
/** Class encapsulating T-Notes startup and shutdown.
...
/** Main T_Notes application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (T_NotesGUI)
...
{
    window = new T_NotesGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &T_NotesGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &T_NotesGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(T_NotesGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(T_NotesGUI::DEFAULT_WALLET);
        ...
        // T_Notes: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &T_NotesGUI::handlePaymentRequest);
        connect(window, &T_NotesGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. T-Notes can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
int main(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(t_notes_locale);
    Q_INIT_RESOURCE(t_notes);
    ...
    /// 6. Determine availability of data directory and parse t-notes.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("T-Notes"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("T-Notes"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // t_notes: links repeatedly have their payment requests routed to this process:
