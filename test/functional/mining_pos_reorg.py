...
class ReorgStakeTest(t_notesTestFramework):
    ...
    def check_money_supply(self, expected_tnc):
        # verify that nodes have the expected TNC supply
        tnc_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(tnc_supply, [DecimalAmt(expected_tnc)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that TNC supply was properly updated after the reorgs
        self.log.info("Check TNC supply...")
