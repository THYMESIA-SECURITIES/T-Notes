...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/T_NotesParams"
    else
        PARAMS_DIR="$HOME/.t_notes-params"
    fi
fi
...
Failed to fetch the T_Notes zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
T_Notes - fetch-params.sh

This script will fetch the T_Notes zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common T_Notes zkSNARK parameters. Note that it is
